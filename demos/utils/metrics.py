...
def compute_metrics(ret_metrics, error_types, metric_names, eps=1e-10, weights=None):
    ...
    if 'accuracy' in metric_names:
        denominator = (error_types['tp_i'] + error_types['fp_i'] + error_types['fn_i'] + error_types['tn_i'])

        if denominator != 0:
            accuracy = np.mean((error_types['tp_i'] + error_types['tn_i']) / denominator)
            ret_metrics['accuracy'].append(accuracy)
        else:
            ret_metrics['accuracy'].append(0)  # or np.nan, or any value you'd want in case of a zero denominator
