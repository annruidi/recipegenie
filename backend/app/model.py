...
import random
import numpy as np
from app.modules.encoder import EncoderCNN, EncoderLabels
from app.modules.transformer_decoder import DecoderTransformer
from app.modules.multihead_attention import MultiheadAttention
from app.utils.metrics import softIoU, MaskedCrossEntropyCriterion
import pickle
import os
...
def mask_from_eos(ids, eos_value, mult_before=True):
    mask = torch.ones(ids.size(), dtype=torch.bool).to(device)
    mask_aux = torch.ones(ids.size(0), dtype=torch.bool).to(device)
